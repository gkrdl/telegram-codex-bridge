...
- `src/codexSessions.js`: Reads Codex's `session_index.jsonl` and returns recent sessions.
...
The bridge only accepts configured Telegram sender IDs. It uses an explicit workspace allowlist and defaults to `/Users/hak`. Dangerous sandbox bypass is not enabled. Secrets are read from environment variables or token files and are never written to logs.
